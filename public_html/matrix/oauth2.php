...
if(isset($_GET["login"]) and in_array($_GET["login"], array("google","logout"))) $login=$_GET["login"];
//else die("wtf");
session_start();
...
if(isset($login) && $login=="logout"){
    ...
    header("Location: http://namesport.co.uk/matrix"); die();
}
if(isset($login)){
    # the following line does the authentication heavy-lifting 
    $a = Auth2::legs(3) -> set(array_slice($u[$login], 0, 4)) -> authorize($u[$login]["auth"]) -> access($u[$login]["token"]) -> finally(function($data) use(&$access_token) { $access_token = $data['access_token']; });
    ...
    $a -> GET($u[$login]["URL"].$access_token) -> then(function($response) {
        ...
        switch($login){
            case "facebook": { $id=$j["id"]; $name=$j["name"]; $email=$j["email"]; break; }
            case "google": { $id=$j["id"]; $name=$j["displayName"]; $email=$j["emails"][0]["value"]; break; }
            case "yandex": { $id=$j["id"]; $name=$j["real_name"]; $email=$j["default_email"]; break; }
            case "vk": { $id=$j["response"][0]["uid"]; $name=$j["response"][0]["first_name"]." ".$j["response"][0]["last_name"]; $email=""; break; }
            case "live": { $id=$j["id"]; $name=$j["name"]; $email=$j["emails"]["preferred"]; break; }
        }
        $_SESSION["id"]=$id;
        $_SESSION["name"]=$name;
    ...
    if(in_array($_SESSION["email"], array("mart.randala@gmail.com", "regdyn@gmail.com")))
        header("Location: http://namesport.co.uk/matrix/");
    else
        header("Location: http://namesport.co.uk/matrix/?login=logout");
}
...
Login with:
<br />
<!--<a href='login.php?login=facebook'>Facebook</a>-->
<a href='/matrix/?login=google'>Google</a>
    <?php
